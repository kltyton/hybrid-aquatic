...
class HydrothermalVentFeature(codec: Codec<HydrothermalVentFeatureConfig>?) :
    ...
    override fun generate(context: FeatureContext<HydrothermalVentFeatureConfig>): Boolean {
        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            val blockState = HybridAquaticBlocks.HYDROTHERMAL_VENT.defaultState
            val blockState2 = HybridAquaticBlocks.HYDROTHERMAL_VENT_SHAFT.defaultState
            ...
            for (l in 0..k) {
                if (structureWorldAccess.getBlockState(blockPos2)
                    ...
                    if (blockState.canPlaceAt(structureWorldAccess, blockPos3) && !structureWorldAccess.getBlockState(
                            blockPos3.down()
                        ).isOf(HybridAquaticBlocks.HYDROTHERMAL_VENT)
                    ) {
                        structureWorldAccess.setBlockState(
                            blockPos3,
                            blockState.with(KelpBlock.AGE, random.nextInt(4) + 20) as BlockState,

...
class DenseKelpColdFeature(codec: Codec<DenseKelpColdFeatureConfig>?) :
    Feature<DenseKelpColdFeatureConfig>(codec) {
    override fun generate(context: FeatureContext<DenseKelpColdFeatureConfig>): Boolean {
        var i = 0
        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            val blockState = HybridAquaticBlocks.DENSE_KELP.defaultState
            val blockState2 = HybridAquaticBlocks.DENSE_KELP_PLANT.defaultState
            ...
            for (l in 0..k) {
                if (structureWorldAccess.getBlockState(blockPos2)
                    ...
                    if (blockState.canPlaceAt(structureWorldAccess, blockPos3) && !structureWorldAccess.getBlockState(
                            blockPos3.down()
                        ).isOf(HybridAquaticBlocks.DENSE_KELP)
                    ) {
                        structureWorldAccess.setBlockState(
                            blockPos3,
                            blockState.with(KelpBlock.AGE, random.nextInt(4) + 20) as BlockState,
